...
except ImportError:  # OpenAI 仅在配置 API Key 时才是必需依赖。
    OpenAI = None

from utils.prompt_loader import PromptLoader
from utils.run_logger import log_llm_exchange
...
"""
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        if OpenAI is None:
            raise RuntimeError("已配置 DEEPSEEK_API_KEY，但未安装 openai；请执行 pip install -r requirements.txt")

        # DeepSeek 提供 OpenAI 兼容接口，只需设置其服务地址。
        client = OpenAI(api_key=self.api_key, base_url=self.BASE_URL)
        try:
            response = client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个严谨的数据处理代码生成器。必须严格遵循字段白名单和动作描述；字符串前后缀拼接只能基于原始输入字段一次完成，绝不递归处理或重复拼接已输出字段。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.1,
            )
            content = response.choices[0].message.content or ""
            log_llm_exchange("DeepSeek", self.model, prompt, content)
        except Exception as error:
            log_llm_exchange("DeepSeek", self.model, prompt, f"[调用失败] {error}")
            raise
        code_match = re.search(r"```python\s*(.*?)```", content, re.DOTALL)
        if code_match:
            return code_match.group(1).strip()
        # 有些模型会遗漏开头 ```python 而保留结尾 ```；清理后再交给沙箱。
        return content.strip().removeprefix("```python").removeprefix("```").removesuffix("```").strip()
    ...
    def compile_rule(self, rule: Rule) -> str:
        """将一条规则编译为可复用的本地代码。"""
        if not self.api_key:
            code = self.local_orchestrate([rule.action_description])
            log_llm_exchange("本地兜底（未调用模型）", "local", self._build_rule_prompt(rule), code)
            return code
    ...
    def understand_json(self, prompt: str) -> str:
        ...
        if not self.api_key or OpenAI is None:
            raise RuntimeError("语义任务需要已配置的 DEEPSEEK_API_KEY 和 openai 依赖")
        client = OpenAI(api_key=self.api_key, base_url=self.BASE_URL)
        response = client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "你是订单语义分析器，只输出合法 JSON 对象。"},
                {"role": "user", "content": prompt},
            ],
        ...
        content = response.choices[0].message.content or "{}"
        log_llm_exchange("DeepSeek-语义任务", self.model, prompt, content)
        return content
