...
from order_processor.domain.order import OrderInput, OrderOutput, ProcessResult
from order_processor.domain.rule import Rule
from order_processor.infrastructure.excel.excel_reader import ExcelReader
from order_processor.infrastructure.excel.excel_writer import ExcelWriter
...
from order_processor.infrastructure.processing.rule_engine import RuleEngine
from order_processor.infrastructure.processing.task_router import TaskRouter
...
class OrderWorkflow:
    ...
    def process_row(self, row: dict) -> ProcessResult:
        ...
        if result["success"]:
            ...
            if not self.rule_repository:
                raise RuntimeError("输出字段必须从 rules.db 的 erp_fields 表读取")
            requested_outputs = self.rule_repository.output_columns(str(row.get("客户代码") or "COMMON").strip())
    ...
    def _rule_engine_for(self, row: dict) -> RuleEngine:
        """按 Excel 的“客户代码”字段加载该客户及通用规则。"""
        if not self.rule_repository:
            return self.rule_engine
        customer_code = str(row.get("客户代码") or "COMMON").strip()
        return RuleEngine(self.rule_repository.load_active_rules(customer_code))
